...
| 6 | Floors.astro — `openLightbox()` | 4.1.2 (AA) | Dynamically injected lightbox container has no `role="dialog"`, `aria-modal="true"`, or accessible name. SR users receive no dialog announcement and virtual cursor is not constrained. | After `container` is created: `container.setAttribute('role','dialog')`, `container.setAttribute('aria-modal','true')`, `container.setAttribute('aria-label', source.getAttribute('data-lightbox-title') \|\| 'Gallery')`. | Purely additive ARIA on a JS-created div — zero visual change. JS edit only. | proposed | |
| 7 | Floors.astro — `openLightbox()` / `closeLightbox()` | 2.4.3 (AA), 2.1.2 (A) | No focus management: focus not moved into lightbox on open, not returned to trigger on close, no focus trap within lightbox. | On open: store `document.activeElement`, call `.focus()` on close button inside container, implement Tab/Shift-Tab trap. On close: return focus to stored element. | JS-only; zero visual change. | proposed | |
| 8 | Floors.astro — SVG `<rect>` (lightbox triggers) + `.floors__floor-mobile-item` `<div>` | 2.1.1 (A) | Click-triggered lightbox openers are keyboard-inaccessible: no `tabindex`, no `role`, no `keydown` Enter/Space handler. | Add `tabindex="0"` + `role="button"` + `aria-label={floor.text}` to each `<rect>` and mobile `<div>`; add `keydown` Enter/Space handler in `init()` alongside existing `click` listener. | `tabindex`/`role`/`aria-label` are purely additive; no visual change. | proposed | |
